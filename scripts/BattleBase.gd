## The reusable BATTLE COORDINATOR — the base every specific battle inherits from. It owns the
## whole fight: spawning units, the per-unit turn flow, the MENU/MOVE/ATTACK input state machine,
## path planning, combat presentation, and the win/lose check. A concrete battle is a thin
## subclass — the demo is `Main extends BattleBase`; a future scripted fight is
## `Battle5 extends BattleBase` that loads its own `Encounter` and layers reactive logic on top.
## (This was carved out of the old single `Main.gd` "God node" — same code, renamed so battles
## can inherit it. Its own decomposition into `TurnManager`/`Encounter`/etc. is the ongoing
## "toward a reusable Battle.tscn" work in docs/TODO.md.)
##
## The turn flow it runs:
...
##
## Whose turn it is now lives in `TurnManager` (the first node carved out of this coordinator
## toward a reusable Battle.tscn): it picks the active unit by FFT-style Charge Time
...
## Optional `res://` path to an `Encounter` `.tres` for this battle. When set (via the Inspector,
## a subclass, or the quick-test hand-off), `_ready` loads that encounter — its map, its enemies,
## and its deploy/win regions — INSTEAD of the hardcoded demo above. Empty (the default) keeps the
## legacy demo battle: the `SmallDemoMap` fallback + `_enemy_roster` + `PartyLoadout` start tiles.
## A scripted battle subclass overrides `_resolve_encounter()` rather than setting this.
@export var encounter_path: String = ""

## The resolved `Encounter` for this battle, or `null` for the legacy demo path. Cached in `_ready`
## so the spawn logic and the win-condition check read the same one. See `_resolve_encounter`.
...
func _ready() -> void:
	...
	# End-of-turn: used by the encounter win-check ("an ally ended its turn on a goal tile").
	...
	# Resolve which fight to run: an authored `Encounter` (its map + enemies + regions) or, when
	# there is none, the built-in demo. Cached so the win-check reads the same one. This loads the
	# encounter's map over the fallback the Battlefield already built (child readied before us).
...
## Turn-end hook — the reach-the-goal half of the win condition. If this encounter defines a `win`
## region and the unit that just finished its turn is a *player* standing on one of those tiles,
## the battle is won. (Elimination — the other half — is handled by `_check_battle_end` on each
## death, so with a win region EITHER reaching the goal OR wiping the enemies wins.) A no-op for the
## demo battle, which has no encounter/win region, and once the game is already over. Fires mid-
## `end_turn`; the subsequent actor hand-off is harmlessly ignored (`_on_active_unit_changed` bails
## on `_game_over`).
func _on_turn_ended(unit: Unit) -> void:
	if _game_over or _encounter == null or not _encounter.has_win_region():
		return
	if not is_instance_valid(unit) or unit.allegiance != Unit.Allegiance.PLAYER:
		return
	if unit.grid_coord in _encounter.region(Encounter.REGION_WIN):
		_end_battle(true)
...
## Return the `Encounter` this battle should run, or `null` to use the built-in demo setup.
## Base behavior: load the `encounter_path` export if one is set (how a hand-authored `.tres`, an
## Inspector assignment, or — later — the designer's quick-test hand-off points us at a fight). A
## scripted battle overrides this to `return Encounter.load_from(...)` for its own file, and can
## layer reactive logic on top. Kept a method (not just the export read inline) so it's the single
## overridable seam every future battle hooks into.
func _resolve_encounter() -> Encounter:
	if encounter_path.is_empty():
		return null
	var enc := Encounter.load_from(encounter_path)
	if enc == null:
		push_warning("BattleBase: could not load encounter '%s' — using demo setup." % encounter_path)
	return enc


## Spawn every unit for the battle. With no encounter this is the legacy demo (party from
## `PartyLoadout` at their stored tiles + the hardcoded `_enemy_roster`); with one it loads the
## encounter's map and spawns from it instead. Split out of `_ready` so a subclass can wrap it.
func _spawn_roster() -> void:
	if _encounter == null:
		# Legacy demo: players from the shared party roster (carrying their chosen loadout);
		# enemies rolled from class+level into recruits.
		...
		return
	_spawn_from_encounter()


## Load the authored encounter: swap in its terrain map, deploy the party into its `deploy` region,
## then spawn its enemies. The party still comes from `PartyLoadout` (who the squad is + their gear
## is a between-battles concern, not the encounter's) — the encounter only decides WHERE they start.
...
func _spawn_from_encounter() -> void:
	...
	# fine for the first cut; per-character start-tile picking within the zone is a later flow.
	var deploy := _encounter.region(Encounter.REGION_DEPLOY)
	var i := 0
	for entry in PartyLoadout.party:
		var tile: Vector2i = deploy[i] if i < deploy.size() else Vector2i(entry["x"], entry["z"])
		_spawn_recruit(tile.x, tile.y, Unit.Allegiance.PLAYER, entry["recruit"])
		i += 1

	# Enemies: one rolled recruit per placement, at its authored tile/class/level. (Per-stat
	# overrides + weapon/armor kits are Phase 3b — `EnemyPlacement.overrides` is unread for now.)
	for placement in _encounter.enemies:
